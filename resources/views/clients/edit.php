  ...
  <form id="client-edit-form" action="/clientes/<?= e($client['id']) ?>" method="POST" class="space-y-6">
    ...
    <div class="rounded-2xl border border-white/5 bg-white/[0.03] p-6 space-y-5">
      ...
      <div>
        ...
        <input aria-label="Nome" type="text" name="name" value="<?= old('name', $client['name']) ?>" required
    ...
    <!-- Visual do Portal -->
    <div class="rounded-2xl border border-white/5 bg-white/[0.03] p-6 space-y-5">
      <h2 class="text-sm font-semibold uppercase tracking-widest text-gray-400">Visual do Portal</h2>
      <div>
        ...
        <input type="url" name="logo_url" value="<?= old('logo_url', $client['logo_url'] ?? '') ?>"
               ...
               class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 transition-colors">
        <p class="mt-1 text-xs text-gray-400">Exibido no portal de aprovação do cliente. URL pública (PNG, SVG, JPG).</p>
    ...
    <div class="rounded-2xl border border-white/5 bg-white/[0.03] p-6 space-y-4" x-data="{enabled: <?= !empty($client['portal_token']) ? 'true' : 'false' ?>}">
      <h2 class="text-sm font-semibold uppercase tracking-widest text-gray-400">Portal do Cliente</h2>
      ...
      <label class="flex items-start gap-3 cursor-pointer">
        ...
        <span>
          ...
          <span class="block text-xs text-gray-400 mt-0.5">O cliente acessa planos, faturas e contratos pelo link. Desativar revoga o acesso imediatamente.</span>
        </span>
      </label>
      ...
      <div x-show="enabled" x-transition style="display:none">
        ...
        <div class="flex items-center gap-2 rounded-xl bg-white/[0.03] border border-white/10 px-3 py-2.5">
          ...
          <button type="button" x-data="{c:false}"
                  ...
                  :class="c ? 'text-emerald-400' : 'text-gray-400 hover:text-white'"
                  class="flex-shrink-0 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <!-- `d` reativo: <template> dentro de <svg> é HTML inválido e quebra o Alpine -->
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    :d="c ? 'M5 13l4 4L19 7' : 'M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z'"/>
            </svg>
          </button>
        </div>
        <?php else: ?>
    ...
    <div class="rounded-2xl border border-white/5 bg-white/[0.03] p-6 space-y-5">
      <h2 class="text-sm font-semibold uppercase tracking-widest text-gray-400">Notificações & automações</h2>
      ...
      <div>
        ...
        <input type="text" name="whatsapp" value="<?= old('whatsapp', $client['whatsapp'] ?? '') ?>"
               ...
               class="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 transition-colors">
        <p class="mt-1 text-xs text-gray-400">Com DDI + DDD. Necessário para os avisos por WhatsApp.</p>
      </div>
      ...
      <div class="pt-2 border-t border-white/[0.06]">
        ...
        <p class="text-xs text-gray-400 mb-3">Só disparam se a automação estiver ativada na agência (em Automações).</p>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          ...
          <label class="flex items-start gap-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 cursor-pointer hover:bg-white/[0.04] transition-colors">
            ...
            <span>
              ...
              <span class="block text-xs text-gray-400"><?= e($def['description'] ?? '') ?></span>
            </span>
          </label>
          <?php endforeach; ?>
        </div>
    ...
    <div x-data="{ open: false }">
      ...
      <div x-show="open" x-cloak x-transition.opacity @keydown.escape.window="open = false" @click.self="open = false"
        ...
        <div class="w-full max-w-md rounded-2xl bg-[#16161f] border border-white/10 shadow-xl p-6">
          ...
          <?php else: ?>
          <p class="text-xs text-gray-400 mb-5">Este cliente ainda não tem faturas, planos ou arquivos vinculados.</p>
          <?php endif; ?>
