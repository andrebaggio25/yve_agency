...
<div class="max-w-2xl mb-6">
  ...
  <?php if ($user): ?>
  <p class="text-sm text-gray-500 mt-0.5"><?= e($user['email']) ?></p>
  <?php endif; ?>
</div>
...
<div class="max-w-2xl space-y-5">
  ...
  <form method="POST" action="<?= $user ? '/admin/usuarios/' . $user['id'] : '/admin/usuarios' ?>" class="card p-6 space-y-5">
    ...
    <div>
      ...
      <select name="agency_id" required class="input-field w-full">
      ...
      <div class="grid grid-cols-2 gap-2 mt-1">
        ...
        <label class="flex items-center gap-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2.5 cursor-pointer hover:bg-white/[0.04] transition-colors">
          ...
          <div>
            ...
            <p class="text-xs text-gray-600"><?= e($role['slug']) ?></p>
          </div>
        </label>
        <?php endforeach; ?>
        <?php if (empty($roles)): ?>
        <p class="text-sm text-gray-600 col-span-2">Nenhum perfil global disponível.</p>
        <?php endif; ?>
      </div>
    </div>
  ...
  <div class="card p-6">
    <p class="text-xs font-semibold uppercase tracking-widest text-gray-500 mb-4">Status da conta</p>
  ...
  <div class="card p-6 space-y-5">
    <p class="text-xs font-semibold uppercase tracking-widest text-gray-500">Senha</p>
