...
<div class="flex items-start justify-between gap-4 mb-6 flex-wrap">
  <div>
    ...
    <p class="text-sm text-gray-400 mt-0.5">Tudo que o sistema enviou por WhatsApp e e-mail nos últimos 30 dias.</p>
  </div>
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
  ...
  foreach ($cards as [$label, $value, $cls]): ?>
    <div class="card p-4">
      <p class="text-xs text-gray-400 mb-1"><?= e($label) ?></p>
...
<form method="GET" class="flex items-end gap-3 mb-4 flex-wrap">
  <div>
    ...
    <select aria-label="Canal" id="channel" name="channel" class="input-field w-40 py-2">
  ...
  <div>
    ...
    <select aria-label="Situação" id="status" name="status" class="input-field w-40 py-2">
...
<?php if (empty($deliveries)): ?>
  ...
  <div class="card p-12 text-center">
    ...
    <p class="text-xs text-gray-400 mt-1">Quando uma automação enviar WhatsApp ou e-mail, o registro aparece aqui.</p>
  </div>
<?php else: ?>
      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-white/[0.04]">
          ...
          <tr class="hover:bg-white/[0.02] transition-colors">
            ...
            <td class="px-4 py-3 text-gray-400"><?= e((string) $d['template']) ?></td>
            <td class="px-4 py-3">
