...
<div class="max-w-2xl mx-auto">
  <div class="mb-8">
    <a href="/usuarios/perfis/<?= e($role['id']) ?>" class="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-white transition-colors mb-4">
      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
      <?= e($role['name']) ?>
    </a>
    <h1 class="text-2xl font-bold text-white"><?= t('roles.edit') ?></h1>
  </div>
  ...
  <form action="/usuarios/perfis/<?= e($role['id']) ?>" method="POST" class="space-y-6">
    <?= csrf_field() ?>
    <?= method_field('PUT') ?>
    ...
    <div class="rounded-2xl border border-white/5 bg-white/[0.03] p-6 space-y-5">
      <div>
        ...
        <input aria-label="Nome" type="text" name="name" value="<?= old('name', $role['name']) ?>" required
    ...
    <?php
      ...
      $activePerms = array_column($role['permissions'] ?? [], 'slug');
    ?>
    ...
    <div class="flex items-center justify-between">
      <?php if (\App\Support\Auth::can('roles.delete')): ?>
      <form action="/usuarios/perfis/<?= e($role['id']) ?>" method="POST" class="inline"
            onsubmit="return confirm('<?= t('roles.confirm_delete') ?>')">
        <?= csrf_field() ?>
        <?= method_field('DELETE') ?>
      ...
      <div class="flex items-center gap-3">
        <a href="/usuarios/perfis/<?= e($role['id']) ?>" class="rounded-xl border border-white/10 px-5 py-2.5 text-sm text-gray-400 hover:text-white transition-colors">
          <?= t('common.cancel') ?>
        </a>
        <button type="submit"
