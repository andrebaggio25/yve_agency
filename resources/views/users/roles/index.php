...
<div>
  <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    <div>
      <p class="text-xs font-semibold uppercase tracking-widest text-violet-500 mb-1"><?= t('nav.settings') ?></p>
  ...
  <div class="flex flex-col items-center justify-center py-24 text-center">
    ...
    <?php if (\App\Support\Auth::can('roles.create')): ?>
    <a href="/usuarios/perfis/novo" class="mt-4 text-sm text-violet-400 hover:text-violet-300 transition-colors">
      <?= t('roles.new') ?> →
    </a>
    <?php endif; ?>
  </div>
  ...
  <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
    ...
    <div class="group rounded-2xl border border-white/5 bg-white/[0.03] p-5 hover:border-violet-500/20 transition-all">
      ...
      <div class="mt-4 flex gap-2">
        ...
        <a href="/usuarios/perfis/<?= e($role['id']) ?>/editar" class="flex-1 text-center rounded-lg bg-violet-500/10 px-3 py-1.5 text-xs text-violet-300 hover:bg-violet-500/20 transition-colors">
          <?= t('common.edit') ?>
        </a>
      </div>
      <?php endif; ?>
