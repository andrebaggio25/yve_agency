...
<div class="min-h-screen">
  <div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
    ...
    <a href="/contratos/novo" class="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-violet-500/20 hover:bg-violet-500 hover:scale-105 active:scale-95 transition-all">
  ...
  <form method="GET" class="mb-6 flex flex-wrap gap-3 items-end">
    <input type="text" name="q" value="<?= e($filters['q'] ?? '') ?>" placeholder="Buscar título..."
      class="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:border-violet-500 focus:outline-none w-56">
  ...
  <div class="flex flex-col items-center justify-center py-24 text-center">
    ...
    <?php if (\App\Support\Auth::can('contracts.create')): ?>
    <a href="/contratos/novo" class="mt-4 text-sm text-violet-400 hover:text-violet-300 transition-colors">Criar primeiro contrato →</a>
    <?php endif; ?>
  </div>
  <?php else: ?>
    ...
    <table class="w-full text-sm">
      ...
      <tbody class="divide-y divide-white/[0.04]">
        ...
        <tr class="group hover:bg-white/[0.03] transition-colors">
          <td class="px-5 py-4">
            <a href="/contratos/<?= $c['id'] ?>" class="font-medium text-white hover:text-violet-300 transition-colors"><?= e($c['title']) ?></a>
          </td>
