...
### QA-01 · Zerar PHPStan nível 6 — `M` 🟡
- **Arquivos:** `app/Core/Response.php`, `Controller.php` (`new static()`), `Core/View.php`, `Core/Container.php`, `Core/Request.php`.
- **Ação:** `new self()`/generics para `new static()`; corrigir tipos em `View::render` e `Container`. Elevar meta para nível 7 depois.
- **Pronto quando:** `composer analyse` retorna 0 erros.

### QA-02 · Testes de autorização e multi-tenancy — `G` 🟠
- **Arquivos:** `tests/Feature/*` (novos).
- **Ação:** para cada módulo sensível, teste positivo e negativo: usuário sem permissão recebe 403; usuário de agência A não lê dado de agência B; `client_ids` limita a visão. Cobrir portal token inválido/desabilitado.
- **Pronto quando:** matriz de permissão coberta; teste falha se alguém remover um `requirePermission`.
