...
### CONT-07 · Reagendar post para outra semana — `P` 🟠 · ✅ **FEITO (2026-09-03)**
> **Problema:** a cliente aprovava o post da semana e pedia para publicar na seguinte. Não havia caminho: a data do post era travada na semana do plano (`assertDateInPlanWeek`), então a pessoa refazia o post do zero no outro plano — plataforma, formato, legenda, roteiro, CTA, capa, imagens, responsável, tudo na mão. E o histórico de aprovação ficava para trás.
> **Entregue:** ação **Reagendar** no post (card e modal de edição). Escolhe a data nova, com atalhos +1/+2/−1 semana. Se a data cai na mesma semana, só troca a data. Se cai em outra, o post **muda de planificação** levando tudo consigo — é a mesma linha trocando de plano, não uma cópia, e os `content_feedbacks` acompanham. **Se a semana de destino ainda não tem planificação, ela nasce ali**, em rascunho, só com o post movido (o modelo semanal do cliente não é aplicado: quem move um post não pediu uma grade de posts vazios). O modal diz para onde o post vai **antes** de confirmar, consultando `GET /conteudo/semana/{clientId}?date=`.
> **Guardas:** semana de destino já aprovada pela cliente **bloqueia** o movimento (não se enfia post novo num plano que ela fechou); status do post é preservado — adiar a data não desfaz a aprovação; escopo por agência e `content.edit` no backend; `activity_logs` grava `content_item.rescheduled` e `content_plan.created_for_reschedule`.
> **Arquivos:** `ContentPlanService::rescheduleItem/planForWeek`, `ContentPlanRepository::findByClientWeek/maxSortOrder/moveItemToPlan`, `ContentPlanController::rescheduleItem/weekPlan`, rotas pt+en com CSRF, `content/show.php`, `public/js/content-editor.js`, `tests/Feature/ContentPlanRescheduleTest.php` (8 testes).
